...
3. **Candidate Pool** — For deep reviews, run `feynman research candidate-pool "<topic>"` after taxonomy and before source acceptance. Save or verify `outputs/.drafts/<slug>-candidate-pool.md`; use it to choose accepted sources, DOI-normalized records, open-access leads, and citation-network candidates. If the command is unavailable or fails, record the failure in the plan and continue with search/web evidence.
